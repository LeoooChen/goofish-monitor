...
设置页里的 `AI 请求间隔（秒）` 会作用在任务批量分析阶段，也就是每个商品调用 AI 之前都会按这个间隔等待。单次 AI 测试只验证 API 地址、Key 和模型是否可用，不代表批量运行时一定不会触发 429 限流。

如果使用智谱/BigModel 等接口时遇到 `429 Too Many Requests`，建议把间隔调到 `5-10` 秒；留空会按默认 `3` 秒处理。
